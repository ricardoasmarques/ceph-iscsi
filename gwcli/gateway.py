...
class ISCSITarget(UIGroup):
    ...
    def ui_command_create(self, target_iqn):
        ...
        # Check there aren't any targets defined manually
        local_lio = root.RTSRoot()
        current_target_names = [tgt.wwn for tgt in local_lio.targets]
        ui_root = self.get_ui_root()
        config_target_names = ui_root.config['targets'].keys()
        if not set(current_target_names).issubset(set(config_target_names)):
            self.logger.error("Local LIO instance already has LIO configured "
                              "with targets - unable to continue")
            return
